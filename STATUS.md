...
| macOS floating state surface | Passing | Installed v0.57.0 Signal Blades system renders distinct Listening, Finishing, Locked, and Needs Attention formations; the contact sheet was generated by the installed shipping renderer and visually inspected |
...
| macOS Fn hotkey | Automated pass; physical confirmation pending | Installed v0.57.0 stays alive after a normal app open; the microphone-free `local-voice-hotkey-diagnostic.v1` probe observed exactly one Fn down and one Fn up through the real event tap |
...
| macOS wake recovery | Implemented; physical wake confirmation pending | v0.57.0 re-arms Core Audio and asynchronously re-warms the selected persistent local engine after macOS wake, but this session did not sleep the operator's Mac |
...
| macOS release artifact | Passing | Installed v0.57.0, strict local code-signature verification, installed/artifact SHA-256 match (`5382ba2025cb2557ff115b72be0e09a7e19b9feee6fcc192aed294e018b23fa2`) |
